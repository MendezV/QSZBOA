...
{
	double tau=*t;
	printf("%f %f %f \n",tau/Tt, Tt*Tt*Omegsq( a, Tt, Tt), Len(a, Tt, Tt));
	for (int i=4; i < (n-4); i++) {
		double x = (i-1)*dx;
		
		
		
		double HRe1 = V(x,V0,a,Tt,Tt)*Re[i] - 0.5*(Re[i+1] - 2*Re[i] + Re[i-1])/ (dx2);
		double HIm1 = V(x,V0,a,Tt,Tt)*Im[i] - 0.5*(Im[i+1] - 2*Im[i] + Im[i-1])/(dx2);
		double HRe1p = V(x,V0,a,Tt,Tt)*Re[i+1] - 0.5*(Re[i+2] - 2*Re[i+1] + Re[i])/ (dx2);
		double HIm1p = V(x,V0,a,Tt,Tt)*Im[i+1] - 0.5*(Im[i+2] - 2*Im[i+1] + Im[i])/(dx2);
		double HRe1m = V(x,V0,a,Tt,Tt)*Re[i-1] - 0.5*(Re[i] - 2*Re[i-1] + Re[i-2])/ (dx2);
		double HIm1m = V(x,V0,a,Tt,Tt)*Im[i-1] - 0.5*(Im[i] - 2*Im[i-1] + Im[i-2])/(dx2);
		double HRe1pp = V(x,V0,a,Tt,Tt)*Re[i+2] - 0.5*(Re[i+3] - 2*Re[i+2] + Re[i+1])/ (dx2);
		double HIm1pp = V(x,V0,a,Tt,Tt)*Im[i+2] - 0.5*(Im[i+3] - 2*Im[i+2] + Im[i+1])/(dx2);
		double HRe1mm = V(x,V0,a,Tt,Tt)*Re[i-2] - 0.5*(Re[i-1] - 2*Re[i-2] + Re[i-3])/ (dx2);
		double HIm1mm = V(x,V0,a,Tt,Tt)*Im[i-2] - 0.5*(Im[i-1] - 2*Im[i-2] + Im[i-3])/(dx2);
		double HRe1ppp = V(x,V0,a,Tt,Tt)*Re[i+3] - 0.5*(Re[i+4] - 2*Re[i+3] + Re[i+2])/ (dx2);
		double HIm1ppp = V(x,V0,a,Tt,Tt)*Im[i+3] - 0.5*(Im[i+4] - 2*Im[i+3] + Im[i+2])/(dx2);
		double HRe1mmm = V(x,V0,a,Tt,Tt)*Re[i-3] - 0.5*(Re[i-2] - 2*Re[i-3] + Re[i-4])/ (dx2);
		double HIm1mmm = V(x,V0,a,Tt,Tt)*Im[i-3] - 0.5*(Im[i-2] - 2*Im[i-3] + Im[i-4])/(dx2);
		
		
		double HRe2 = V(x,V0,a,Tt,Tt)*(Re[i]-HIm1*dt*0.5) - 0.5*((Re[i+1]-HIm1p*dt*0.5) - 2*(Re[i]-HIm1*dt*0.5) + (Re[i-1]-HIm1m*dt*0.5))/ (dx2);
		double HIm2 = V(x,V0,a,Tt,Tt)*(Im[i]+HRe1*dt*0.5) - 0.5*((Im[i+1]+HRe1p*dt*0.5) - 2*(Im[i]+HRe1*dt*0.5) + (Im[i-1]+HRe1m*dt*0.5))/(dx2);
		double HRe2p = V(x,V0,a,Tt,Tt)*(Re[i+1]-HIm1p*dt*0.5) - 0.5*((Re[i+2]-HIm1pp*dt*0.5) - 2*(Re[i+1]-HIm1p*dt*0.5) + (Re[i]-HIm1*dt*0.5))/ (dx2);
		double HIm2p = V(x,V0,a,Tt,Tt)*(Im[i+1]+HRe1p*dt*0.5) - 0.5*((Im[i+2]+HRe1pp*dt*0.5) - 2*(Im[i+1]+HRe1p*dt*0.5) + (Im[i]+HRe1*dt*0.5))/(dx2);
		double HRe2m = V(x,V0,a,Tt,Tt)*(Re[i-1]-HIm1m*dt*0.5) - 0.5*((Re[i]-HIm1*dt*0.5) - 2*(Re[i-1]-HIm1m*dt*0.5) + (Re[i-2]-HIm1mm*dt*0.5))/ (dx2);
		double HIm2m = V(x,V0,a,Tt,Tt)*(Im[i-1]+HRe1m*dt*0.5) - 0.5*((Im[i]+HRe1*dt*0.5) - 2*(Im[i-1]+HRe1m*dt*0.5) + (Im[i-2]+HRe1mm*dt*0.5))/(dx2);
		double HRe2pp = V(x,V0,a,Tt,Tt)*(Re[i+2]-HIm1pp*dt*0.5) - 0.5*((Re[i+3]-HIm1ppp*dt*0.5) - 2*(Re[i+2]-HIm1pp*dt*0.5) + (Re[i]-HIm1*dt*0.5))/ (dx2);
		double HIm2pp = V(x,V0,a,Tt,Tt)*(Im[i+2]+HRe1pp*dt*0.5) - 0.5*((Im[i+3]+HRe1ppp*dt*0.5) - 2*(Im[i+2]+HRe1pp*dt*0.5) + (Im[i]+HRe1*dt*0.5))/(dx2);
		double HRe2mm = V(x,V0,a,Tt,Tt)*(Re[i-2]-HIm1mm*dt*0.5) - 0.5*((Re[i]-HIm1*dt*0.5) - 2*(Re[i-2]-HIm1mm*dt*0.5) + (Re[i-3]-HIm1mmm*dt*0.5))/ (dx2);
		double HIm2mm = V(x,V0,a,Tt,Tt)*(Im[i-2]+HRe1mm*dt*0.5) - 0.5*((Im[i]+HRe1*dt*0.5) - 2*(Im[i-2]+HRe1mm*dt*0.5) + (Im[i-3]+HRe1mmm*dt*0.5))/(dx2);
		
		
		double HRe3 = V(x,V0,a,Tt,Tt)*(Re[i]-HIm2*dt*0.5) - 0.5*((Re[i+1]-HIm2p*dt*0.5) - 2*(Re[i]-HIm2*dt*0.5) + (Re[i-1]-HIm2m*dt*0.5))/ (dx2);
		double HIm3 = V(x,V0,a,Tt,Tt)*(Im[i]+HRe2*dt*0.5) - 0.5*((Im[i+1]+HRe2p*dt*0.5) - 2*(Im[i]+HRe2*dt*0.5) + (Im[i-1]+HRe2m*dt*0.5))/(dx2);
		double HRe3p = V(x,V0,a,Tt,Tt)*(Re[i+1]-HIm2p*dt*0.5) - 0.5*((Re[i+2]-HIm2pp*dt*0.5) - 2*(Re[i+1]-HIm2p*dt*0.5) + (Re[i-1]-HIm2m*dt*0.5))/ (dx2);
		double HIm3p = V(x,V0,a,Tt,Tt)*(Im[i+1]+HRe2p*dt*0.5) - 0.5*((Im[i+2]+HRe2pp*dt*0.5) - 2*(Im[i+1]+HRe2p*dt*0.5) + (Im[i-1]+HRe2m*dt*0.5))/(dx2);
		double HRe3m = V(x,V0,a,Tt,Tt)*(Re[i-1]-HIm2m*dt*0.5) - 0.5*((Re[i]-HIm2*dt*0.5) - 2*(Re[i-1]-HIm2m*dt*0.5) + (Re[i-2]-HIm2mm*dt*0.5))/ (dx2);
		double HIm3m = V(x,V0,a,Tt,Tt)*(Im[i-1]+HRe2m*dt*0.5) - 0.5*((Im[i]+HRe2*dt*0.5) - 2*(Im[i-1]+HRe2m*dt*0.5) + (Im[i-2]+HRe2mm*dt*0.5))/(dx2);
		
		
		
		double HRe4 = V(x,V0,a,Tt,Tt)*(Re[i]-HIm3*dt) - 0.5*((Re[i+1]-HIm3p*dt) - 2*(Re[i]-HIm3*dt) + (Re[i-1]-HIm3m*dt))/ (dx2);
		double HIm4 = V(x,V0,a,Tt,Tt)*(Im[i]+HRe3*dt) - 0.5*((Im[i+1]+HRe3p*dt) - 2*(Im[i]+HRe3*dt) + (Im[i-1]+HRe3m*dt))/(dx2);
...
{
	double V00=0;
	double delta2 = 1/10000;
	double arg1 = 0.5*pow(x, 2)/delta2;
	double e1 = V00*exp(-arg1);
	double arg2 = 0.5*pow(x - Len( a, tau, Tt), 2)/delta2;
	double e2 = V00*exp(-arg2);
